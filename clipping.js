...
function cdAuthInit() {
  AUTH.deviceId = _authDeviceId();
  const saved = localStorage.getItem('tk_admin_token');
  if (saved) {
    AUTH.token = saved;
    _authHideOverlay();
    cdConnectSSE();
...
async function cdLoadLocalVideo() {
  ...
  if (!api?.takneekPickLocalVideo) {
    cdStatus('Electron API not available', 'err');
    return;
  }
  ...
  if (!filePath) {
    cdStatus('File selection cancelled');
    return;
  }
...
function cdDrop(e) {
  ...
  if (!filePath) {
    cdStatus('Could not get file path. Use the Load Local button instead.', 'err');
    return;
  }
...
const VideoStreamService = (() => {
  ...
  function connect(token) {
    ...
    fetch(`${ADMIN_API}/video-processing/stream`, {
      ...
      if (res.status === 401) { cdAuthLogout(); return; }
      if (res.status === 404) {
        _store.clear();
        _notify();
...
function cdFetchVideos() {
  if (!AUTH.token) { _authShowOverlay(); return; }
...
function renderClips() {
  ...
  list.innerHTML = CD.clips.map((clip, i) => {
    ...
    const annBadge = isBowler && clip.annotations.length > 0
      ? `<span class="cd-ann-badge">${clip.annotations.length} stage${clip.annotations.length > 1 ? 's' : ''}</span>`
      : '';

    const stagesHtml = isBowler && selected && clip.annotations.length > 0
...
function renderAnnPanel() {
  ...
  if (CD.category !== 'bowler') { panel.classList.remove('visible'); return; }
  ...
  if (!CD.selectedClipId) {
    ...
    document.getElementById('cd-ann-header').textContent = 'Bowling Stages';
    return;
  }
  ...
  document.getElementById('cd-ann-header').textContent = clip.label;
  ...
  stagesEl.innerHTML = STAGE_ORDER.map((stage, idx) => {
    ...
    if (isDone) {
      if (isRange) {
        valueHtml = `<span class="cd-srow-val">${ann.frameStart} → ${ann.frameEnd}</span>`;
      } else {
        valueHtml = `<span class="cd-srow-val">frame ${ann.frame}</span>`;
      }
    }
    ...
    if (isDone) {
      actionHtml = `
        <button class="cd-srow-redo" onclick="cdClearStage('${clip.id}','${stage}')" title="Re-mark this stage">↺</button>`;
    } else if (isRange) {
      actionHtml = `
        <button class="cd-srow-mark cd-srow-start" onclick="cdMarkStage('${stage}','start')" title="Mark start at current frame">▶ Start</button>
        <button class="cd-srow-mark cd-srow-end" onclick="cdMarkStage('${stage}','end')" title="Mark end at current frame">■ End</button>`;
    } else {
      actionHtml = `
        <button class="cd-srow-mark" onclick="cdMarkStage('${stage}','frame')" title="Mark at current frame">◆ Mark</button>`;
    }
      ...
      <div class="cd-srow${isDone ? ' done' : ''}">
        <div class="cd-srow-left">
          ...
          <div class="cd-srow-info">
            <span class="cd-srow-label">${label}</span>
            ${valueHtml}
          </div>
        </div>
...
function cdSetCategory(cat) {
  CD.category = cat;
  document.getElementById('cd-cat-batsman').className = `cd-cat-btn${cat === 'batsman' ? ' active' : ''}`;
  document.getElementById('cd-cat-bowler').className = `cd-cat-btn${cat === 'bowler' ? ' active' : ''}`;
  renderClips(); renderAnnPanel();
  cdGenerateClips(true);
  cdStatus(`Category → ${cat === 'batsman' ? 'Batsman' : 'Bowler'}`);
}
...
async function cdUploadClips() {
  if (!CD.generated) { cdStatus('Add and mark clips first', 'err'); return; }
  ...
  // Legacy path — old Takneek API
  if (!CD.activeVideo.player_id) { cdStatus('No player linked to this video', 'err'); btn.disabled = false; return; }
...
function updateUploadBtn() {
  document.getElementById('cd-upload-btn').disabled = !CD.generated || CD.clips.length === 0;
}
...
function _renderPlayersTable(videos) {
  ...
  tbody.innerHTML = players.map(p => {
    ...
    return `
      <tr>
        <td class="cd-ptable-name">${escHtml(p.name)}</td>
        ${numCell(p.batter)}
        ${numCell(p.bowler)}
